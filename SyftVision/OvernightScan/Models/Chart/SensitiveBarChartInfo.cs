...
{
    class SensitiveBarChartInfo : ChartInfo
    {
        public SensitiveBarChartInfo(Global.BatchType batchType, string chartName, string subCharName, Dictionary<string, List<TargetScanInfo>> targetScanInfoListDic, bool lastBatchSelectionEnable = true, bool summaryEnable = false)
        ...
        public Dictionary<string, Dictionary<string, List<BarXY>>> GetXYData()
        {
            ...
            foreach (var batch in this.Batches)//batches - batch
            {
                ...
                foreach (var scan in batch.Value)//scans - scan
                {
                    ...
                    {
                        RP_Data rpData = scan.GetRP_Data(componentInfo.RPCode, this.ChartConfig.ScanPhase, Scan.FastMode.Sensitivity);
                        ...
                        components.Add(new BarXY(rpData.Reagent, rpData.Product, rpData.Sensitivity(), componentInfo.Max, componentInfo.Min, this.ChartConfig.Accept));
                    }
        ...
        private XYChart SingleBatchXYChartGenerator(Dictionary<string, List<BarXY>> scans, string batchName, int firstScanID, int lastScanID, int scanCount)
        {
            ...
            if (SummaryEnable)
            {
                ...
                foreach (var X in scans.First().Value.Select(a => a.X).ToList())
                {
                    ...
                    foreach (var scan in scans)
                    {
                        ...
                        {
                            if (item.X == X && item.Y != 0) meanList.Add(Math.Round(item.Y, 2));
                        }
                    }
            ...
            else
            {
                ...
                foreach (var scan in scans)
                {
                    ...
                    layer.addDataSet(scan.Value.Select(a => Math.Round(a.Y, 2)).ToArray(), Global.ColorPool[i++], $"{scan.Key}");
                }
            }
            ...
            BarLayer blayer1 = c.addBarLayer(scans?.First().Value.Select(a => Double.Parse(a.Min)).ToArray(), unchecked((int)0x80ff8080));
